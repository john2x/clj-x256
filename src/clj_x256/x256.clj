...
(defn hex-to-rgb
  [hex]
  [(Integer/parseInt (subs hex 0 2) 16)
...
(def ^:const xterm-colors
  (vec (map hex-to-rgb xterm-colors-hex)))
...
(defn from-rgb
  ...
  ([c]
   (let [distance-map 
           ...
           (reduce merge (map #(hash-map (.indexOf xterm-colors %2)
                                         (distance %1 %2))
